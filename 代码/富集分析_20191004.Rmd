...
barplot(ego,showCategory = 20,title = "enrichGO result")
dim(ego) #3996 10
dim(ego[ego$ONTOLOGY=='BP',]) #3509 10
dim(ego[ego$ONTOLOGY=='MF',]) #251 10
dim(ego[ego$ONTOLOGY=='CC',]) #236 10
write.csv(as.data.frame(ego),'E:/enrichment_analysis/enrichGO.csv',row.names = FALSE)
```
3.KEGG Analysis
```{r}
kk<-enrichKEGG(as.character(geneList$Entrez))
dim(kk) #158 9
barplot(kk,showCategory = 20,title = "enrichKEGG result")
kk<-setReadable(kk,OrgDb = org.Hs.eg.db,keyType = 'ENTREZID')
write.csv(as.data.frame(kk),'E:/enrichment_analysis/enrichKEGG.csv',row.names = FALSE)
```
...
write.csv(as.data.frame(formula_kk),file = 'E:/enrichment_analysis/formula_enrichKEGG.csv',row.names = FALSE)
dotplot(formula_kk,x = ~Mutation_Level,title = "formula enrichKEGG result")+ggplot2::facet_grid(~Copy_Number_Alteration)
