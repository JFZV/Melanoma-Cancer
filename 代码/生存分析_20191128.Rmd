...
SKCM.surv.cut<-surv_cutpoint(SKCM.surv,time = "times", event = "patient.vital_status", variables = c("BRAF","CDKN2A","SPP1"))

SKCM.surv.cat<-surv_categorize(SKCM.surv.cut)
```

2.对BRAF, CDKN2A, SPP1分别进行生存分析
```{r}
pdf("ggsurv_survival_CDKN2A_SPP1.pdf",onefile = FALSE);
fit<-surv_fit(Surv(times,patient.vital_status)~CDKN2A+SPP1,data = SKCM.surv.cat);
ggsurvplot(fit,
           risk.table = TRUE,
           pval = TRUE,
           conf.int = FALSE,
...
3.三个基因一起生存分析
```{r}
pdf("ggsurv_survival_BRAF_CDKN2A_SPP1.pdf",onefile = FALSE);
fit<-surv_fit(Surv(times,patient.vital_status)~BRAF+CDKN2A+SPP1,data = SKCM.surv.cat);
ggsurvplot(fit,
           risk.table = TRUE,
           pval = TRUE,
           conf.int = FALSE,
